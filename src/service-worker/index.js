...
self.addEventListener("fetch", event => {
  ...
  if (!isPassageLookup(url)) return;

  const fromDb = isEsv ? fromEsvDb(url) : fromKjvDb(url);

  event.respondWith(
    fromDb
      .then(text => {
        if (isEsv) {
          const esvApiJson = {passages: [text]};
          text = JSON.stringify(esvApiJson);
        }
        log(
          `Serving ${url.pathname}${url.search} from db`,
