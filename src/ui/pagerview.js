...
class Page extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (
      this.root != null &&
      (prevProps.index !== index || this.root.scrollHeight > this.rootHeight)
    ) {
...
export default class PagerView extends Component {
  ...
  calculateVelocity(touch, timeStamp) {
    ...
    if (touchMove == null || moveTime == null)
      return (
        Math.abs(touch.screenX - touchStart.screenX) / (timeStamp - startTime)
      );

    return Math.abs(touch.screenX - touchMove.screenX) / (timeStamp - moveTime);
  }

  calculateSlope(touch) {
    ...
    return Math.abs(
      (touch.screenY - previousTouch.screenY) /
  ...
  touchOffset() {
    if (this.state.touchMove == null || this.state.touchStart == null) return 0;
    else return this.state.touchMove.screenX - this.state.touchStart.screenX;
  }
  ...
  offset() {
    if (
      ...
      this.state.touchDirection !== "horizontal"
    )
      return this.restingOffset();
    else return this.restingOffset() + this.touchOffset();
  }
  ...
  newIndex() {
    ...
    if ((isFlick && offset > 0) || offset > window.innerWidth / 2)
      if (this.props.index > 0) return this.props.index - 1;
      else return this.props.index;
    else if ((isFlick && offset < 0) || offset < -(window.innerWidth / 2))
      if (this.props.index < CHAPTER_COUNT - 1) return this.props.index + 1;
      else return this.props.index;
    else return this.props.index;
  }
  ...
  updateCurrentPage() {
    ...
    if (index !== this.props.index) this.props.onIndexChange(index);
  ...
  componentDidMount() {
    this.pageScrollDepths[this.props.index] = this.props.getInitialScroll();
  }
