  ...
  while (index++ < html.length) {
    ...
    if (ctx.type === "document") {
      ...
      if (isAttribteEnd) {
        ...
        context.pop();
        if (!ctx.isQuoted && html[index] === ">") index--;
      }
    }
  }
...
export function* stripTags(tagName, tagsAndText) {
  ...
  for (let item of tagsAndText) {
    if (item.type === "tag" && item.isStart && item.name === tagName) {
      inObjectTag = true;
    } else if (item.type === "tag" && !item.isStart && item.name === tagName) {
      inObjectTag = false;
    } else if (!inObjectTag) {
      yield item;
    }
...
export function* addSpansAroundVerses(tagsAndText) {
  ...
  for (let item of tagsAndText) {
    if (inVerse && item.type === "tag" && item.name === "p" && !item.isStart) {
      yield {
        type: "tag",
        isStart: false,
        value: "</span>",
...
export function* withTagStack(tagsAndText) {
  ...
  for (let item of tagsAndText) {
    ...
    else yield {...item, stack: stack.slice(0)};

    if (item.type === "tag")
      if (item.isStart) stack.push(item);
      else stack.pop();
  }
}
...
export function* addDropCapsClassToFirstLetter(tagsAndTextWithStack) {
  ...
  for (let item of tagsAndTextWithStack) {
    ...
    if (
      ...
      if (leadingWhitespace) yield {type: "text", value: leadingWhitespace};
      ...
      if (leadingEntities) yield {type: "text", value: leadingEntities};
      else yield {type: "text", value: firstLetter};
      ...
      if (rest)
        if (leadingEntities) yield {type: "text", value: firstLetter + rest};
        else yield {type: "text", value: rest};
...
export function* stripCopyright(tagsAndTextWithSurroundingEvents) {
  ...
  for (let item of tagsAndTextWithSurroundingEvents) {
    ...
    } else if (item.type === "tag" && item.attributes.class === "copyright") {
      // Don't yield this item
      inTag = true;
    } else if (inTag) {
      if (item.type === "tag" && !item.isStart) inTag = false;
    } else if (
      item.type === "text" &&
      item.value[0] === ")" &&
        ...
        addDropCapsClassToFirstLetter(
          withTagStack(
            addSpansAroundVerses(
              stripTags("script", stripTags("object", tagsAndText(text)))
            )
          )
        )
      )
